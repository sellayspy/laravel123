...
class CustomerController extends Controller
{
    ...
    {
      $customer = Customer::orderBy('id_customer', 'asc')->get();

       return datatables()
       ->of($customer)
       ->addIndexColumn()
       ->addColumn('kode_member', function($customer){
        return '<span class="badge badge-success">'. $customer->kode_member .'</span>';
       })
    ...
    public function store(Request $request)
    {
      ...
      $kodeMember = $customer->kode_member +1 ?? 1;

      $customer = new Customer();
      $customer->kode_member = tambah_nol_didepan($kodeMember, 5);
